import React, { useEffect, useState, useRef } from "react";
import axios from "axios";
import Navbar from "./Navbar.js";
import { message, Modal } from "antd";
import csvtojson from "csvtojson";
import SingleDomain from "../components/Domain/SingleDomain.js";
import MultipleDomain from "../components/Domain/MultipleDomain.js";
import { useNavigate } from "react-router-dom";

const URL = "http://localhost:8080";

const Dashboard = () => {

  const navigate = useNavigate();


  const [showSingleeDom, setShowSingleDom] = useState(false);
  const [domians , setDomains] = useState([]);



const handleDelete = async (name) => {
  try {
    console.log(JSON.stringify([{ Name: name }]));

    const response = await axios.post(`${URL}/api/v1/domain/delete`, [{ Name: name}], {
      headers: {
        'Content-Type': 'application/json'
      }
    });

    console.log(response)

    if (response.status === 204) {
      
      
      message.success(`${name} deleted successfully`);
      
    } else {
      message.error('Failed to delete domain');
      
    }
  } catch (error) {
    message.error('Error occurred: ' + error.message);
  }
};






  useEffect(()=>{

    const fetchData = async () => {
      try {
        const { data } = await axios.get(`http://localhost:8080/api/v1/domain/all`);
        
        setDomains(data);
  
        console.log(data)
      
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

      fetchData();


  },[])









  return (
    <>
      <Navbar />

      <div className="text-gray-900 h-400 bg-gray-200">
        <div className="p-11 flex justify-center">
          <h1 className="text-3xl font-bold">Domain Management</h1>
        </div>

        {showSingleeDom ? <SingleDomain /> : <MultipleDomain />}

        <div
          class="order-2 md:order-3 flex flex-wrap items-center justify-end mr-0 md:mr-4"
          id="nav-content"
        >
          <div class="auth flex items-center w-full md:w-full">
            <button
              class="bg-blue-600 text-gray-200  p-2 rounded mr-3  hover:bg-blue-500 hover:text-gray-100"
              onClick={() => setShowSingleDom(true)}
            >
              Add single domain
            </button>
            <button
              class="bg-blue-600 text-gray-200  p-2 rounded  hover:bg-blue-500 hover:text-gray-100"
              onClick={() => setShowSingleDom(false)}
            >
              Add Multiple domain
            </button>
          </div>
        </div>

        <div className="overflow-x-auto p-5">



          




        
          <table className="min-w-full bg-white">
            <thead className="bg-gray-700">
              <tr>
                <th className="pl-6 w-8">
                  <input
                    id="checkbox"
                    type="checkbox"
                    className="hidden peer"
                  />
                  <label
                    htmlFor="checkbox"
                    className="relative flex items-center justify-center p-0.5 peer-checked:before:hidden before:block before:absolute before:w-full before:h-full before:bg-white w-5 h-5 cursor-pointer bg-blue-500 border border-gray-400 rounded overflow-hidden"
                  >
                    {/* SVG omitted for brevity */}
                  </label>
                </th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-white">
                  Domain Name
                </th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-white">
                  Records
                </th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-white">
                  Host ID
                </th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-white">
                  Add DNS
                </th>
                <th className="px-6 py-3 text-left text-sm font-semibold text-white">
                  Actions
                </th>
              </tr>
            </thead>

            {domians.map((value,index)=>(

            <tbody>
              <tr className="even:bg-blue-50">
                <td className="pl-6 w-8">
                  <input
                    id={`checkbox`}
                    type="checkbox"
                    className="hidden peer"
                    onChange={() => console.log("Checkbox Changed")} // Placeholder function
                  />
                  <label
                    htmlFor={`checkbox`}
                    className="relative flex items-center justify-center p-0.5 peer-checked:before:hidden before:block before:absolute before:w-full before:h-full before:bg-white w-5 h-5 cursor-pointer bg-blue-500 border border-gray-400 rounded overflow-hidden"
                  >
                    {/* SVG omitted for brevity */}
                  </label>
                </td>
                <td className="px-6 py-4 text-sm">{value.Name}</td>
                <td className="px-6 py-4 text-sm">{value.ResourceRecordSetCount}</td>
                <td className="px-6 py-4 text-sm">{value.Id.split('/').pop()}</td>
                <td className="px-6 py-4 text-sm">
                  <div
                    class="order-2 md:order-3 flex flex-wrap items-center justify-end mr-0 md:mr-4"
                    id="nav-content"
                  >
                    <div class="auth flex items-center w-full md:w-full">
                      <button class="bg-blue-600 text-gray-200  p-2 rounded  hover:bg-blue-500 hover:text-gray-100" onclick={navigate(`dns/${value.Name}/${value.Id.split('/').pop()}`)  }>
                        ADD
                      </button>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <button className="mr-4" title="Edit">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-5 fill-blue-500 hover:fill-blue-700"
                      viewBox="0 0 348.882 348.882"
                    >
                      <path
                        d="m333.988 11.758-.42-.383A43.363 43.363 0 0 0 304.258 0a43.579 43.579 0 0 0-32.104 14.153L116.803 184.231a14.993 14.993 0 0 0-3.154 5.37l-18.267 54.762c-2.112 6.331-1.052 13.333 2.835 18.729 3.918 5.438 10.23 8.685 16.886 8.685h.001c2.879 0 5.693-.592 8.362-1.76l52.89-23.138a14.985 14.985 0 0 0 5.063-3.626L336.771 73.176c16.166-17.697 14.919-45.247-2.783-61.418zM130.381 234.247l10.719-32.134.904-.99 20.316 18.556-.904.99-31.035 13.578zm184.24-181.304L182.553 197.53l-20.316-18.556L294.305 34.386c2.583-2.828 6.118-4.386 9.954-4.386 3.365 0 6.588 1.252 9.082 3.53l.419.383c5.484 5.009 5.87 13.546.861 19.03z"
                        data-original="#000000"
                      />
                      <path
                        d="M303.85 138.388c-8.284 0-15 6.716-15 15v127.347c0 21.034-17.113 38.147-38.147 38.147H68.904c-21.035 0-38.147-17.113-38.147-38.147V100.413c0-21.034 17.113-38.147 38.147-38.147h131.587c8.284 0 15-6.716 15-15s-6.716-15-15-15H68.904C31.327 32.266.757 62.837.757 100.413v180.321c0 37.576 30.571 68.147 68.147 68.147h181.798c37.576 0 68.147-30.571 68.147-68.147V153.388c.001-8.284-6.715-15-14.999-15z"
                        data-original="#000000"
                      />
                    </svg>
                  </button>

                  <button className="mr-4" title="Delete" onClick={()=>handleDelete(value.Name)}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="w-5 fill-red-500 hover:fill-red-700"
                      viewBox="0 0 24 24"
                    >
                      <path
                        d="M19 7a1 1 0 0 0-1 1v11.191A1.92 1.92 0 0 1 15.99 21H8.01A1.92 1.92 0 0 1 6 19.191V8a1 1 0 0 0-2 0v11.191A3.918 3.918 0 0 0 8.01 23h7.98A3.918 3.918 0 0 0 20 19.191V8a1 1 0 0 0-1-1Zm1-3h-4V2a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v2H4a1 1 0 0 0 0 2h16a1 1 0 0 0 0-2ZM10 4V3h4v1Z"
                        data-original="#000000"
                      />
                      <path
                        d="M11 17v-7a1 1 0 0 0-2 0v7a1 1 0 0 0 2 0Zm4 0v-7a1 1 0 0 0-2 0v7a1 1 0 0 0 2 0Z"
                        data-original="#000000"
                      />
                    </svg>
                  </button>
                </td>
              </tr>
            </tbody>
            ))}
          </table>




        </div>
      </div>
    </>
  );
};

export default Dashboard;
